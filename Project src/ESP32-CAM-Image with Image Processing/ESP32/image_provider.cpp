...
extern String test;  //extern --> Use variable from other file
...
{
  fb = esp_camera_fb_get(); // fb is a pointer
  if (!fb)
  {
...
TfLiteStatus PerformCapture(tflite::ErrorReporter *error_reporter, int image_width, int image_height, int channels, uint8_t *image_data)
{
  ...
  int ret = capture_image(); // return status of capture operation
  
  if (ret != 0) // if capture fail
  {
    return kTfLiteError;
  }
  ...
  {
    memcpy(image_data, fb->buf, fb->len); // image data
  }
  else
  {
    // Trimming Image
    ...
    /* Encode image */
  String encoded = base64::encode(fb->buf, fb->len); //fb --> pointer get data
  test=encoded; //declare variable

  
  

  
  esp_camera_fb_return(fb); // return memory of fb
